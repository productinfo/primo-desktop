  ...
  const svg = (type, width = 6) =>
    ({
      vercel: `<svg viewBox="0 0 284 65" fill="currentColor" style="width: ${width}rem;height: 100%;"
            ...
            /></svg
          >`,
      netlify: `<?xml version="1.0" encoding="UTF-8" standalone="no"?>
<svg
...
<div class="boxes">
  ...
  {:else if showingHosts && !hostBeingConnected}
    <div class="hosts">
      ...
      <div class="buttons" in:fade={{ duration: 200 }}>
        <button on:click={() => (hostBeingConnected = 'netlify')}>
          {@html svg('netlify')}</button
        >
      </div>
    </div>
  {/if}
...
<style lang="postcss">
  ...
  .boxes {
    display: grid;
    gap: 1rem;
    /* make the boxes next to each other in row */
